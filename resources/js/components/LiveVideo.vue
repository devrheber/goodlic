    ...
    <div class="agora-box">
      <div class="agora-input">
        <div class="agora-button">
          <button
            type="button"
            v-if="this.User.role == 'Mentor' && this.appointment_type_id == 6 "
...
export default {
  ...
  mixins: [loginMixin],
  props: ["id", "appointment_type_id", "mentee_id"],
  ...
  methods: {
    async joinEvent() {
      ...
      if (this.appointment_type_id == 2 || this.appointment_type_id == 6) {
  ...
  created() {
    this.checkLoggedIn();
    this.option.appid = process.env.MIX_AGORA_APP_ID
